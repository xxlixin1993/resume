...
(function($){
	$.fn.extend({easyResponsiveTabs:function(options){
		var defaults={
				type:'default',width:'auto',fit:true
				}
		...
		this.each(function(){
			...
			var $tabItemh2;$respTabs.find('.resp-tab-content').before("<h2 class='resp-accordion hi-icon-wrap hi-icon-effect-5 hi-icon-effect-5a' role='tab'><i class='hi-icon'></i></h2>");
			var itemCount=0;
			$respTabs.find('.resp-accordion').each(function(){
				$tabItemh2=$(this);$tabItemh2.find(".hi-icon").addClass(tab_icons[itemCount]);
			...
			$respTabs.find('.resp-tab-item').each(function(){
				...
				var tabcount=0;$respTabs.find('.resp-tab-content').each(function(){
					$tabContent=$(this);
					$tabContent.attr('aria-labelledby','tab_item-'+(tabcount));tabcount++;
					});
				count++;
				});
			$respTabs.find("[role=tab]").each(function(){
				...
				$currentTab.click(function(){
					...
					if($currentTab.hasClass('resp-accordion')&&$currentTab.hasClass('resp-tab-active')){
						...
						$currentTab.removeClass('resp-tab-active');return false;
						}
